...
import {UpdateUserRequest, User} from '@src/lib/dto';
import {useMutation} from '@tanstack/react-query';
import {queryClient, stayeaseAxios} from '@src/lib/client';
...
const Profile = () => {
  ...
  const updateUserMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      toast.error(
        error.response.data.address || 'Error updating profile details',
      );
    },
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <section className="mt-8 flex flex-wrap gap-10">
        ...
        <div className="space-y-2 w-3/12">
          <label>Contact: </label>
          <Input placeholder="+19999999999" {...form.register('phoneNumber')} />
        </div>
        ...
        <div className="flex gap-5">
          ...
          <Button onClick={onCancel} disabled={updateUserMutation.isPending}>
            Cancel
          </Button>
        </div>
      </section>
