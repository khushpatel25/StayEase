...
import {PropertyBookRequest} from '@src/lib/dto';
...
function calculateTaxAmount(amount: number) {
  const taxRate = 0.15;
...
export const PaymentPage = () => {
  ...
  const bookingDetails = {
    ...
    id: +(search.get('id') || 0),
    taxes: calculateTaxAmount(+(search.get('total') || 0)),
  };

  const totalAmount = bookingDetails.price + bookingDetails.taxes;
          ...
          <div>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex flex-col items-end">
                  <span className="font-bold text-lg">
                    CAD {bookingDetails.price.toFixed(2)}
                  </span>
                  <span className="text-sm text-gray-600">
                    +CAD {bookingDetails.taxes.toFixed(2)} taxes and charges
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FaDollarSign />
                <span>
                  Excludes CAD {bookingDetails.taxes.toFixed(2)} in taxes and
                  other charges
                </span>
              </div>
              <div className="ml-6 text-sm">
                <div>
                  ...
                  <span className="float-right">
                    CAD {bookingDetails.taxes.toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
